...
function Generate_extrastats($mid, $el) {
  ...
  $e = mysqli_query($link, "SELECT t1.*,t2.league, t3.longname FROM $matchstats_table t1 JOIN ".$matches_table." t2 ON t2.id=t1.matchid JOIN 2004leagues t3 ON t3.id=t2.league WHERE matchid='".$mid."'");
  if(mysqli_num_rows($e)==0) return false;
  else {
    ...
    while($i < count($es["goalie1"])) {
        if(!isset($es["s1ot"][$i])) $es["s1ot"][$i]=0;
        $ot=$ot+intval($es["s1ot"][$i]); $i++; 
    }
    $i=0;
    $prid1=$prid2="";
    while($i < count($es["goalie2"])) {
    if(!isset($es["s2ot"][$i])) $es["s2ot"][$i]=0;
        $ot=$ot+intval($es["s2ot"][$i]); $i++;
    }
    ...
    while($i < count($es["goalie1"]))
        ...
        if(!isset($es["g1_goals"][$i])) $es["g1_goals"][$i]=0;
        if(!isset($es["g1_shots"][$i])) $es["g1_shots"][$i]=0;
        if(!isset($es["s1p1"][$i])) $es["s1p1"][$i]=0;
        if(!isset($es["s1p2"][$i])) $es["s1p2"][$i]=0;
        if(!isset($es["s1p3"][$i])) $es["s1p3"][$i]=0;
        if(!isset($es["s1ot"][$i])) $es["s1ot"][$i]=0;
    ...
    while($i < count($es["goalie2"]))
        ...
        if(!isset($es["g2_goals"][$i])) $es["g2_goals"][$i]=0;
        if(!isset($es["g2_shots"][$i])) $es["g2_shots"][$i]=0;
        if(!isset($es["s2p1"][$i])) $es["s2p1"][$i]=0;
        if(!isset($es["s2p2"][$i])) $es["s2p2"][$i]=0;
        if(!isset($es["s2p3"][$i])) $es["s2p3"][$i]=0;
        if(!isset($es["s2ot"][$i])) $es["s2ot"][$i]=0;
