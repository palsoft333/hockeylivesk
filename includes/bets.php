<?php
  ...
  if($_GET[uid]) $uid=$_GET[uid];
	...
	if(strtotime($aRow[datetime]) > mktime()) $edit = '<button class=\'btn btn-sm btn-block btn-hl\' onclick=\'location.href=\"/games/'.$aRow[league].'-'.SEOtitle($aRow[longname]).$vl.'\";\' data-toggle=\'tooltip\' data-placement=\'top\' title=\''.LANG_BETS_CHANGE.'\'><i class=\'fas fa-edit\'></i></button>';
  if($_GET[uid])
    {
    if(strtotime($aRow[datetime]) > mktime()) $aRow[tip1]=$aRow[tip2]="?";
    $edit="";
    }
