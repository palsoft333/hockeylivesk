        ...
        switch ($type) {
          ...
          case "conference":
            foreach($this->conference as $key => $conf) {
              ...
              usort($conf_teams[$key]["teams"], function($a,$b){ $c = $b["points"] - $a["points"]; $c .= $b["gdiff"] - $a["gdiff"]; $c .= $a["games"] - $b["games"]; $c .= $b["gf"] - $a["gf"]; $c .= $b["wins"] - $a["wins"]; $c .= $a["losts"] - $b["losts"]; return $c; });
            if($condensed) break;
            }
