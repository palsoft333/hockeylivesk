...
if(isset($_GET["profile"]))
  ...
  if($_SESSION['logged'])
    {
    $q = mysqli_query($link, "SELECT *, JSON_LENGTH(login_session) as num_devices FROM e_xoops_users WHERE uid='".$_SESSION['logged']."'");
    $f = mysqli_fetch_array($q);
    $r = mysqli_query($link, "SET SESSION sql_mode = 'NO_ENGINE_SUBSTITUTION';") or die(mysqli_error($link));
    $r = mysqli_query($link, "SELECT shortname, longname FROM 2004teams WHERE shortname='".$f["user_favteam"]."' GROUP BY shortname UNION SELECT shortname, longname FROM el_teams WHERE shortname='".$f["user_favteam"]."' GROUP BY shortname ORDER BY longname ASC");
      ...
      <div class="card shadow">
        ...
        <div class="card-body">
          <form class="user">
            ...
            <div class="form-group">
              <label>'.LANG_NAV_NOTIFTITLE.'</label>
              <div class="custom-control custom-checkbox">
                <input class="custom-control-input" type="checkbox" name="mailnotify" id="mailnotify"'.($f["mail_notify"]==1 ? " checked" : "").'>
                <label class="custom-control-label" for="mailnotify">'.LANG_USERPROFILE_EMAIL.'</label>
              </div>
              <div class="custom-control custom-checkbox">
                <input class="custom-control-input" type="checkbox" name="pushnotify" id="pushnotify"'.($f["push_id"]!=NULL ? " checked" : "").'>
                <label class="custom-control-label" for="pushnotify">'.LANG_USERPROFILE_PUSH.'</label>
              </div>
            </div>
            <div class="form-group">
  ...
  if($f["posts"]>0) {
    $i = mysqli_query($link, "SELECT c.*, u.uname, u.user_avatar FROM comments c LEFT JOIN e_xoops_users u ON u.uid=c.uid WHERE c.uid='".$_SESSION['logged']."' ORDER BY c.datum DESC LIMIT 5");
...
$content .= '
   ...
   <div class="col-auto flex-grow-1 flex-shrink-1 d-none d-xl-block">
    ...
    $w = mysqli_query($link, "SELECT shortname, longname FROM 2004teams GROUP BY shortname UNION SELECT shortname, longname FROM el_teams GROUP BY shortname ORDER BY longname ASC");
...
if(isset($_GET["notif"]))
  ...
  if(isset($_SESSION['logged']))
    ...
    $f = mysqli_fetch_array($q);
    $w = mysqli_query($link, "SELECT * FROM user_notifications WHERE uid='".$_SESSION['logged']."' ORDER BY datetime DESC LIMIT 20");
    if($f["user_avatar"]!="") $avatar = '/images/user_avatars/'.$_SESSION['logged'].'.'.$f["user_avatar"]."?".filemtime('images/user_avatars/'.$_SESSION['logged'].'.'.$f["user_avatar"]);
    else $avatar = '/img/players/no_photo.jpg';
    ...
    if(mysqli_num_rows($w)>0) $content .= '<p class="text-right"><button class="btn btn-sm btn-hl" id="markread"><i class="fas fa-check-double"></i> '.LANG_NAV_NOTIFMARK.'</button></p>';
    ...
    while($e = mysqli_fetch_array($w))
      {
       if($e["what"]==0)
        ...
        if($el==1) $t = mysqli_query($link, "SELECT * FROM el_matches WHERE id='".$ide."'");
        else $t = mysqli_query($link, "SELECT * FROM 2004matches WHERE id='".$ide."'");
       ...
       elseif($e["what"]==1)
        ...
        $t = mysqli_query($link, "SELECT * FROM 2004leagues WHERE id='".$exp[0]."'");
        $r = mysqli_fetch_array($t);
        $icon = 'plus';
        $color = 'primary';
        $url = '/bets';
       ...
       elseif($e["what"]==3)
        {
        $t = mysqli_query($link, "SELECT c.*, u.uname FROM comments c LEFT JOIN e_xoops_users u ON u.uid=c.uid WHERE c.id='".$e["whatid"]."'");
        $r = mysqli_fetch_array($t);
    ...
    if(mysqli_num_rows($w)==0) $notif .= '<div class="card-body"><p class="dropdown-item text-gray-600">'.LANG_NAV_NOTIFNO.' ...</p></div>';
    $content .= '</div>
   </div> <!-- end col -->
   <div class="col-auto flex-grow-1 flex-shrink-1 d-none d-xl-block">
    '.$advert.'
...
elseif(isset($id))
  {
  $leaguecolor = "hl";
  $q = mysqli_query($link, "SELECT * FROM e_xoops_users WHERE uid='".$id."'");
  if(mysqli_num_rows($q)>0)
    {
    $f = mysqli_fetch_array($q);
    $r = mysqli_query($link, "SET SESSION sql_mode = 'NO_ENGINE_SUBSTITUTION';") or die(mysqli_error($link));
    $r = mysqli_query($link, "SELECT id, shortname, longname, 0 as el FROM 2004teams where shortname='".$f["user_favteam"]."' && RIGHT(longname, 3)!='U20' UNION SELECT id, shortname, longname, 1 as el FROM el_teams WHERE shortname='".$f["user_favteam"]."' ORDER BY id DESC LIMIT 1");
    $t = mysqli_fetch_array($r);
    $y = mysqli_query($link, "SELECT sum(points) as poc FROM `fl_wallet` WHERE uid='".$id."' GROUP BY uid");
    if(mysqli_num_rows($y)>0) $u = mysqli_fetch_array($y);
    $u["poc"] = $u["poc"] ?? 0;
    if($f["tip_points"]>0)
      {
      $i = mysqli_query($link, "SET @rank=0;");
      $i = mysqli_query($link, "SELECT * FROM (SELECT @rank:=@rank+1 AS rank, uid, tip_points FROM e_xoops_users ORDER BY tip_points DESC)dt WHERE dt.uid='".$id."'");
      $o = mysqli_fetch_array($i);
      }
    if($f["avg_time"]!=NULL) {
        $avg = $f["avg_time"]/60;
      ...
      <div class="card shadow animated--grow-in">
        ...
        <div class="card-body">
          <dl class="row">
            ...
            <dd class="col-sm-4 text-hl">'.$u["poc"].'</dd>
  ...
  if($f["posts"]>0) {
    $i = mysqli_query($link, "SELECT c.*, u.uname, u.user_avatar FROM comments c LEFT JOIN e_xoops_users u ON u.uid=c.uid WHERE c.uid='".$id."' ORDER BY c.datum DESC LIMIT 5");
    $num = mysqli_num_rows($i);
    $content .= '
    <div class="card shadow my-4 animated--grow-in">
      <div class="card-header">
