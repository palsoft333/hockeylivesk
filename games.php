...
if(isset($_GET["gid"])) { $gid = explode("-", htmlspecialchars($_GET["gid"])); $gid=$gid[0]; }
...
elseif(isset($gid))
  {
	...
	$a = mysqli_query($link, "SELECT * FROM ".$matches_table." WHERE id='".$ide."'");
	if(mysqli_num_rows($a)>0)
    {
    $b = mysqli_fetch_array($a);
    $c = mysqli_query($link, "SELECT *, dt.arena FROM ".$teams_table." JOIN (SELECT * FROM el_infos)dt ON shortname=dt.teamshort WHERE shortname='".$b["team1short"]."' && league='".$b["league"]."'");
    $d = mysqli_query($link, "SELECT * FROM ".$teams_table." WHERE shortname='".$b["team2short"]."' && league='".$b["league"]."'");
    if(mysqli_num_rows($c)>0 && mysqli_num_rows($d)>0) {
        $t1 = mysqli_fetch_array($c);
        $t2 = mysqli_fetch_array($d);
        $e = mysqli_query($link, "SELECT name, goals, asists FROM ".$players_table." WHERE teamshort='".$b["team1short"]."' && league='".$b["league"]."' ORDER BY points DESC, goals DESC, asists DESC, gwg DESC, gtg DESC, shg DESC, ppg DESC, penalty ASC LIMIT 1");
        $p1 = mysqli_fetch_array($e);
        if(!isset($p1)) { $p1["goals"]="0"; $p1["asists"]="0"; $p1["name"]=""; }
        $f = mysqli_query($link, "SELECT name, goals, asists FROM ".$players_table." WHERE teamshort='".$b["team2short"]."' && league='".$b["league"]."' ORDER BY points DESC, goals DESC, asists DESC, gwg DESC, gtg DESC, shg DESC, ppg DESC, penalty ASC LIMIT 1");
        $p2 = mysqli_fetch_array($f);
        if(!isset($p2)) { $p2["goals"]="0"; $p2["asists"]="0"; $p2["name"]=""; }
        $g = mysqli_query($link, "SELECT count(id) as poc, ROUND(sum(tip1)/count(id),2) as vys1, ROUND(sum(tip2)/count(id),2) as vys2 FROM ".$tips_table." WHERE matchid='".$b["id"]."'");
        ...
        $linf = mysqli_query($link, "SELECT * FROM 2004leagues WHERE id='".$b["league"]."'");
        $linfo = mysqli_fetch_array($linf);
        ...
        $content .= '<i class="float-left h1 h1-fluid ll-'.LeagueFont($linfo["longname"]).' text-gray-600 mr-1"></i>
                ...
                <h2 class="h6 h6-fluid text-'.$leaguecolor.' text-uppercase font-weight-bold mb-3">'.$linfo["longname"].'</h2>
                    ...
                    <div class="col-6 col-md-4 order-2 order-md-1 mt-3 p-fluid">';
                        if($lowerdiv) $i = mysqli_query($link, "(SELECT m.*, 1 as roz, l.longname FROM ".$matches_table." m LEFT JOIN 2004leagues l ON l.id=m.league WHERE (m.team1short='".$b["team1short"]."' || m.team2short='".$b["team1short"]."') && m.kedy='konečný stav' && l.longname LIKE '%".$lowerdiv."%' ORDER BY m.datetime DESC LIMIT 5) UNION (SELECT m.*, 2 as roz, l.longname FROM ".$matches_table." m LEFT JOIN 2004leagues l ON l.id=m.league WHERE (m.team1short='".$b["team2short"]."' || m.team2short='".$b["team2short"]."') && m.kedy='konečný stav' && l.longname LIKE '%".$lowerdiv."%' ORDER BY m.datetime DESC LIMIT 5)");
                        else $i = mysqli_query($link, "(SELECT m.*, 1 as roz, l.longname FROM ".$matches_table." m LEFT JOIN 2004leagues l ON l.id=m.league WHERE (m.team1short='".$b["team1short"]."' || m.team2short='".$b["team1short"]."') && m.kedy='konečný stav' && l.longname NOT LIKE '%U20%' ORDER BY m.datetime DESC LIMIT 5) UNION (SELECT m.*, 2 as roz, l.longname FROM $matches_table m LEFT JOIN 2004leagues l ON l.id=m.league WHERE (m.team1short='".$b["team2short"]."' || m.team2short='".$b["team2short"]."') && m.kedy='konečný stav' && l.longname NOT LIKE '%U20%' ORDER BY m.datetime DESC LIMIT 5)");
                        $z1=$z2=$roz1=$roz2=0;
                        $lastt1=$lastt2=[];
                        ...
                        if($j["roz"]==1)
                            {
                            $lastt1[$z1] = array($j["team1short"], $j["team1long"], $j["team2short"], $j["team2long"], $j["goals1"], $j["goals2"], $j["datetime"], $j["id"], $j["longname"]);
                            $roz1++;
                            $z1++;
                            }
                        else
                            {
                            $lastt2[$z2] = array($j["team1short"], $j["team1long"], $j["team2short"], $j["team2long"], $j["goals1"], $j["goals2"], $j["datetime"], $j["id"], $j["longname"]);
                            $roz2++;
                            $z2++;
                            }
                        }
                        ...
                        $content .= '<div class="border-bottom"><a href="/report/'.$lastt1[$x][7].$el.'-'.SEOtitle($lastt1[$x][1].' vs '.$lastt1[$x][3]).'" class="badge badge-pill badge-'.$color1.'">'.$lastt1[$x][4].':'.$lastt1[$x][5].'</a> vs. '.$vs1.($el==1 ? ' '.$kde1:'').($el==0 ? '<span class="d-block text-xs">('.$lastt1[$x][8].')</span>':'').'</div>';
                        $x++;
                        }
                    $content .= '</div>
                    ...
                    <div class="col-6 col-md-4 order-3 text-right mt-3 p-fluid">';
                        ...
                        $content .= '<div class="border-bottom">vs. '.$vs2.($el==1 ? ' '.$kde2:'').' <a href="/report/'.$lastt2[$x][7].$el.'-'.SEOtitle($lastt2[$x][1].' vs '.$lastt2[$x][3]).'" class="badge badge-pill badge-'.$color2.'">'.$lastt2[$x][4].':'.$lastt2[$x][5].'</a>'.($el==0 ? '<span class="d-block text-xs">('.$lastt2[$x][8].')</span>':'').'</div>';
                        $x++;
                        }
                    $content .= '</div>
                    </div>
                    <div class="row no-gutters my-4">
                    <div class="col-6 col-md-4 order-2 order-md-1 mt-3 p-fluid">';
                        if($lowerdiv) $i = mysqli_query($link, "SELECT m.*, l.longname FROM ".$matches_table." m LEFT JOIN 2004leagues l ON l.id=m.league WHERE (m.team1short='".$b["team1short"]."' && m.team2short='".$b["team2short"]."' && m.kedy='konečný stav' && l.longname LIKE '%".$lowerdiv."%') || (m.team1short='".$b["team2short"]."' && m.team2short='".$b["team1short"]."' && m.kedy='konečný stav' && l.longname LIKE '%".$lowerdiv."%') ORDER BY m.datetime DESC LIMIT 5");
                        else $i = mysqli_query($link, "SELECT m.*, l.longname FROM ".$matches_table." m LEFT JOIN 2004leagues l ON l.id=m.league WHERE (m.team1short='".$b["team1short"]."' && m.team2short='".$b["team2short"]."' && m.kedy='konečný stav' && l.longname NOT LIKE '%U20%') || (m.team1short='".$b["team2short"]."' && m.team2short='".$b["team1short"]."' && m.kedy='konečný stav' && l.longname NOT LIKE '%U20%') ORDER BY m.datetime DESC LIMIT 5");
                        $h2h=[];
                        while($j = mysqli_fetch_array($i))
                        {
                        if($_SESSION["lang"]!='sk') { $j["team1long"] = TeamParser($j["team1long"]); $j["team2long"] = TeamParser($j["team2long"]); }
                        $h2h[] = array($j["team1short"], $j["team1long"], $j["team2short"], $j["team2long"], $j["goals1"], $j["goals2"], $j["datetime"], $j["id"], $j["longname"]);
                        }
                        ...
                        $content .= '<div class="border-bottom"><a href="/report/'.$h2h[$x][7].$el.'-'.SEOtitle($h2h[$x][1].' vs '.$h2h[$x][3]).'" class="badge badge-pill badge-'.$color1.'">'.$g1.'</a> '.$hteam.($el==1 ? ' '.$kde1:'').($el==0 ? '<span class="d-block text-xs">('.$h2h[$x][8].')</span>':'').'</div>';
                        $x++;
                        }
                    $content .= '</div>
                    <div class="col-md-4 order-1 order-md-2 d-flex justify-content-center align-items-center">
                    ...
                    <div class="col-6 col-md-4 order-3 text-right mt-3 p-fluid">';
                        ...
                        $content .= '<div class="border-bottom">'.$ateam.($el==1 ? ' '.$kde2:'').' <a href="/report/'.$h2h[$x][7].$el.'-'.SEOtitle($h2h[$x][1].' vs '.$h2h[$x][3]).'" class="badge badge-pill badge-'.$color2.'">'.$g2.'</a>'.($el==0 ? '<span class="d-block text-xs">('.$h2h[$x][8].')</span>':'').'</div>';
                        $x++;
                        }
                    $content .= '</div>
                    </div>';
                    ...
                    $goa1 = mysqli_query($link, "SELECT *, ROUND((svs/sog)*100,1) as svp FROM $goalies_table WHERE league='".$b["league"]."' && teamshort='".$b["team1short"]."' && IF(gp=1 && (svs/sog)=1, 1, 0)=0 ORDER BY svp DESC LIMIT 1");
                    $goa2 = mysqli_query($link, "SELECT *, ROUND((svs/sog)*100,1) as svp FROM $goalies_table WHERE league='".$b["league"]."' && teamshort='".$b["team2short"]."' && IF(gp=1 && (svs/sog)=1, 1, 0)=0 ORDER BY svp DESC LIMIT 1");
                    $goalie1 = mysqli_fetch_array($goa1);
                    $goalie2 = mysqli_fetch_array($goa2);
                    if(!isset($goalie1)) { $goalie1["svp"]="0"; $goalie1["name"]=""; }
                    if(!isset($goalie2)) { $goalie2["svp"]="0"; $goalie2["name"]=""; }
                    ...
                    $tw1 = ($t1["zapasov"]==0 ? 0:($t1["wins"]/$t1["zapasov"])*($twp/(($dpp+$l5p+$twp+$ptp)/100)));
                    $pt1 = ($t1["zapasov"]==0 ? 0:($t1["body"]/($t1["zapasov"]*$linfo["points"]))*($ptp/(($dpp+$l5p+$twp+$ptp)/100)));
                    $st1 = round($dp1+$l51+$tw1+$pt1,0);
                    $dp2 = 0;
                    $l52 = ($w2/5)*($l5p/(($dpp+$l5p+$twp+$ptp)/100));
                    $tw2 = ($t2["zapasov"]==0 ? 0:($t2["wins"]/$t2["zapasov"])*($twp/(($dpp+$l5p+$twp+$ptp)/100)));
                    $pt2 = ($t2["zapasov"]==0 ? 0:($t2["body"]/($t2["zapasov"]*$linfo["points"]))*($ptp/(($dpp+$l5p+$twp+$ptp)/100)));
