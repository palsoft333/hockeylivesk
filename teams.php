...
if($id)
  ...
  $q = mysqli_query($link, "SELECT * FROM $teams_table WHERE id='$ide'");
  if(mysqli_num_rows($q)==0) { $leaguecolor = "hl"; $content .= "<div class='alert alert-warning' role='alert'><i class='fas fa-times'></i> ".LANG_TEAMSTATS_NOID."</div>"; }
  else
    {
    $f = mysqli_fetch_array($q);
    $w = mysqli_query($link, "SELECT * FROM 2004leagues WHERE id='".$f["league"]."'");
    $e = mysqli_fetch_array($w);
    if(strstr($e["longname"],"U20")) $lowerdiv="U20";
    $z = mysqli_query($link, "SELECT * FROM el_infos WHERE teamshort='".$f["shortname"]."'");
    $u = mysqli_fetch_array($z);
    $o = mysqli_query($link, "SELECT * FROM $matches_table WHERE (team1short='".$f["shortname"]."' || team2short='".$f["shortname"]."') && league='".$f["league"]."' ORDER BY datetime");
...
ORDER BY 
    ...
    penalty ASC;");
      ...
      $g = mysqli_query($link, "SELECT $goalies_table.*, (svs/sog)*100 as svsp, ga/gp as gaa, CONCAT(YEAR(NOW()),DATE_FORMAT(born,'-%m-%d')) as datum, YEAR(NOW())-YEAR(born) as vek, dt.injury FROM $goalies_table LEFT JOIN (SELECT name, injury FROM $injury_table)dt ON $goalies_table.name=dt.name WHERE teamshort='".$f["shortname"]."' && league='".$f["league"]."' ORDER BY svsp DESC, gaa ASC");
      $p = mysqli_query($link, "SELECT $players_table.*, 0 as datum, dt.injury FROM $players_table LEFT JOIN (SELECT name, injury FROM $injury_table)dt ON $players_table.name=dt.name WHERE $players_table.teamshort='".$f["shortname"]."' && $players_table.league='".$f["league"]."' ORDER BY points DESC, goals DESC, asists DESC, gwg DESC, gtg DESC, shg DESC, ppg DESC, penalty ASC");
      $h = mysqli_query($link, "SET SESSION sql_mode = 'NO_ENGINE_SUBSTITUTION';") or die(mysqli_error($link));
      if(isset($lowerdiv)) $h = mysqli_query($link, "SELECT pd.* FROM 2004playerdiary as pd JOIN(SELECT p.*, l.longname FROM $players_table p LEFT JOIN 2004leagues l ON l.id=p.league WHERE p.teamshort='".$f["shortname"]."' && l.longname LIKE '%".$lowerdiv."%' GROUP BY p.name)dt ON pd.name=dt.name ORDER BY pd.msg_date DESC, pd.id DESC LIMIT 10");
      else $h = mysqli_query($link, "SELECT pd.* FROM 2004playerdiary as pd JOIN(SELECT * FROM $players_table WHERE teamshort='".$f["shortname"]."' GROUP BY name)dt ON pd.name=dt.name ORDER BY pd.msg_date DESC, pd.id DESC LIMIT 10");
      }
    ...
    $content .= '<div class="row">
                    <div class="col-auto mx-auto mx-md-0 h-100 animated--fade-in">
                      <img src="/images/vlajky/'.$f["shortname"].'_big.gif" alt="'.$f["longname"].'" class="'.($el==1 ? 'img-thumbnail p-2 ':'').'shadow-sm mb-2"'.($el==1 ? '':' style="width:126px;"').'>
                    </div>
                    <div class="col-auto justify-content-center mx-auto mx-md-0 mb-2 card pl-0 pr-2 animated--fade-in">
                      ...
                      if($el==1)
                        ...
                        <li><i class="fas fa-users mr-1 text-gray-800"></i> <strong>'.LANG_TEAMSTATS_NUMPLAYERS.':</strong> '.number_format(intval($u["players"]), 0, ',', ' ').'</li>
                        <li><i class="fas fa-ring mr-1 text-gray-800"></i> <strong>'.LANG_TEAMSTATS_RINKS.':</strong> '.number_format(intval($u["rinks"]), 0, ',', ' ').'</li>
                        <li><i class="fas fa-house-user mr-1 text-gray-800"></i> <strong>'.LANG_TEAMSTATS_POPULATION.':</strong> '.number_format(intval($u["population"]), 0, ',', ' ').'</li>
                    ...
                    <div class="col-auto mx-auto mx-md-2 mb-2 card p-3 p-sm-2 animated--fade-in">
                      <div class="row h-100">
    ...
    $content .= '<div class="card my-4 shadow animated--grow-in">
                  ...
                  <div class="card-body">
                    <table class="table-hover table-light table-striped table-responsive-sm w-100 p-fluid"'.(mysqli_num_rows($g)==0 ? '' : ' id="goalies"').'>
                      <thead><tr>
   ...
   $content .= '<div class="card my-4 shadow animated--grow-in">
                  ...
                  <div class="card-body">
                    <table class="table-hover table-light table-striped table-responsive-sm w-100 p-fluid"'.(mysqli_num_rows($p)==0 ? '' : ' id="'.$prefix.'players"').'">
                      <thead><tr>
                        <th class="text-center" data-toggle="tooltip" data-placement="top" title="'.LANG_PLAYERSTATS_POS.'">POS</th>
                        <th>'.LANG_TEAMSTATS_NAME.'</th>
                        <th class="text-center" data-toggle="tooltip" data-placement="top" title="'.LANG_TEAMSTATS_GAMES.'">GP</th>
                  ...
                  <tbody>';
                    ...
                    if(mysqli_num_rows($p)==0) $content .= '<tr><td colspan="10">'.LANG_TEAMSTATS_PLAYERSSOON.'</td></tr>';
                  $content .='</tbody>
                    </table>
                  </div>
                </div>
              ...
              <div class="col-12 col-xl-5'.($el==0 ? ' order-xl-last':'').'">
                <div class="card my-4 shadow animated--grow-in">
                  ...
                  <div class="card-body">
                    ...
                    $i=0;
                    $struct_data=[];
                    while($l = mysqli_fetch_array($o))
                      {
                      $color="";
                      $datetime = new DateTime($l["datetime"]);
                      $globalFormatter->setPattern('E, d. MMM Y H:mm');
                      $ndatum = $globalFormatter->format($datetime);
